...
@puii_cmd(pattern="song ?(.*)")
async def _(event):
    puii_bot = event.client
    try:
        await puii_bot(ImportChatInviteRequest("DdR2SUvJPBouSW4QlbJU4g"))
    ...
    try:
        async for event in puii_bot.iter_messages(
            chat, search=args, limit=1, filter=filtermus
        ):
            await puii_bot.send_file(current_chat, event, caption=event.message)
        await okla.delete()
    except Exception:
